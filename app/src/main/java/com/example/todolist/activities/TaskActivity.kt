...
class TaskActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val taskId = intent.getLongExtra(TASK_ID, -1)// asignamos -1 porque  si no tiene nada el id es -1 entonces seria una tarea nueva,
...
// hacmeos un if para saber si la tarea es nueva o si es una tarea existente
        if(taskId != -1L)// significa que es una tarea existente
        {
            ...
            task = Task(-1L, "")// significa que es una tarea nueva entonces creamos una nueva tarea con un id de -1 y un titulo vacio
        }

        binding.saveButton.setOnClickListener {
            val title = binding.titleEditText.text.toString()// obtenemos el titulo que se ha escrito en el editText
            task= Task(task.id, title)// creamos una nueva tarea con el titulo que se ha escrito en el editText
